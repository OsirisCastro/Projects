...
#pragma endregion
	...
	void Update_VulkanRenderer(entt::registry& registry, entt::entity entity)
	{
		...
		std::map<GeometryData, int> geometryInstanceCount;

		auto instances = registry.group<GeometryData>(entt::get<GPUInstance>, entt::exclude<DRAW::DoNotRender>);

		instances.sort<GeometryData>([](const auto& a, const auto& b) {
			return a < b;
		});

		for (auto entity : instances)
		{
			auto& geometry = instances.get<GeometryData>(entity);
			auto& instance = instances.get<GPUInstance>(entity);

			instanceData.push_back(instance);

			auto gData = instances.get<GeometryData>(entity);
			if (geometryInstanceCount.find(gData) == geometryInstanceCount.end())
			{
				geometryInstanceCount[gData] = 1;
			}
			else
			{
				geometryInstanceCount[gData]++;
			}
		}
		...
		if (registry.all_of< VulkanVertexBuffer, VulkanIndexBuffer>(entity))
		{
			...
			for (const auto& [data, instance] : geometryInstanceCount)
			{
				vkCmdDrawIndexed(commandBuffer, data.indexCount, instance, data.indexStart, data.vertexStart, instanceOffset);
				instanceOffset += instance;
			}
